...
MongoData.Provider = partialMemo(function MongoDataProvider({children, contextClass, dataClass, value, proxyRef, onChange, ...props}) {
    ...
    return (
        <ProxyContextProvider {...props} value={valueRef.current} contextClass={contextClass} proxyRef={valueRef} onChange={(...args) => {
            if (isCallable(onChange)) return onChange(...args);
        }}>
            {children}
