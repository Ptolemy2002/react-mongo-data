...
export default class MongoData {
        ...
        Object.keys(this.properties).forEach(name => {
            ...
            if (!property.equals(prevValue)) {
                if (isNullOrUndefined(property.get())) {
                    ...
                    const diff = flattenKeys(differenceFn(property.toMongo(prevValue), property.toMongo(property.get())), name + ".");
                    ...
                    property.get().forEach((value, i) => {
                        const type = MongoData.getType(value);
                        if (!prevValue?.some(v => MongoData.comparePropertyValues(type, property.get(), v)))
                            newValues.push(property.toMongo(property.get())[i]);
                    });

                    property.toMongo(prevValue)?.forEach(value => {
                        const type = MongoData.getType(value);
                        if (!(
                            property.get() ? 
                    ...
                    result.$set[property.mongoName] = property.toMongo(property.get())?.toISOString()
                } else {
                    result.$set[property.mongoName] = property.toMongo(property.get());
                }
            }
        });
    ...
    undo(steps = 1, type=null) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            index = this.lastCheckpointIndex(type, {start: index, includeCurrent: true});
    ...
    defineRequestType(id, run=() => {}, {
        ...
        this.requestTypes[id] = {
            run: async function (...args) {return run.bind(this)(...args)},
            pre: async function (...args) {return pre.bind(this)(...args)},
            post: async function (...args) {return post.bind(this)(...args)}
        };
    ...
    request(id, ...args) {
        ...
        const run = request.pre.bind(this)(...args, ac)
            .then(() => this._initRequest(id))
            .then(() => request.run.bind(this)(...args, ac))
            .then((data) => {
            ...
            .finally(() => request.post.bind(this)(...args, ac));
...
MongoData.useContext = function useDataContext(contextClass, dataClass, deps, onChange, listenReinit=true) {
    ...
    const set = useCallback((v) => _setValue(valueToInstance(dataClass, v)), [_setValue]);
    return new HookResult({value, set}, ["value", "set"]);
}

MongoData.Provider = partialMemo(function MongoDataProvider({children, contextClass, dataClass, value, proxyRef, onChange: _onChange, ...props}) {
    ...
    const onChange = useCallback((...args) => {
        if (isCallable(_onChange)) return _onChange(...args);
    }, [_onChange]);

    return (
        <ProxyContextProvider {...props} value={valueRef.current} contextClass={contextClass} proxyRef={valueRef} onChange={onChange}>
            {children}
