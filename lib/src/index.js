...
MongoData.useContext = function useDataContext(contextClass, deps) {
    return useProxyContext(contextClass, deps);
}
...
MongoData.Provider = function DataProvider({children, contextClass, dataClass, value, ...props}) {
    ...
    if (valueRef.current === undefined) {
        if (value instanceof dataClass) {
            ...
            valueRef.current = dataClass.createFromJSON(value);
        } else {
            valueRef.current = value;
        }
    }
